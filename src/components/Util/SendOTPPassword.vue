...
import VerifyOTPpassword from './VerifyOTPpassword.vue';
...
const memberinfo = computed(() => store.getters.getMember)
onMounted(async () => {
    if (editing.value) {
        await store.dispatch('fetchInfoMember')
        if (memberinfo.value) {
            email.value = memberinfo.value.data.email
        }
    }   
})
...
<template>
    ...
    <VerifyOTPpassword :show="showModalOTPVerify" :email="email" @update:show="showModalOTPVerify = $event" />
</template>
